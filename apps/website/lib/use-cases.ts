...
import { useCasesFr } from './use-cases.fr'
...
export type UseCasePackage = 's3nd' | '@s3nd/react' | '@s3nd/cli' | '@s3nd/protocol'

/** The part of a use case that is the same in every language. */
interface UseCaseBase {
  ...
  /** The full guide, in the documentation. */
  guideHref: string
}
...
export type UseCase = UseCaseBase & UseCaseCopy & { guide: { label: string; href: string } }

const base: UseCaseBase[] = [
  {
...
export async function POST(request: Request) {
  ...
  return Response.json({ code })
    ...
    guideHref: docs('/use-cases/new-device'),
  },
  {
...
try {
  ...
  throw error
    ...
    packages: ['s3nd'],
    guideHref: docs('/use-cases/continuous-backup'),
  },
  {
    slug: 'encrypted-sync',
    kind: 'app-state',
...
await transfers.createSnapshot({
  ...
  version: 3,
    ...
    guideHref: docs('/use-cases/encrypted-sync'),
  },
  {
    slug: 'attachments',
    kind: 'app-state',
...
const url = await store.getUrl(record.attachmentKey, { expiresIn: 600 })`,
    ...
    packages: ['s3nd'],
    guideHref: docs('/use-cases/attachments'),
  },
]

export const useCasesEn: Record<UseCaseSlug, UseCaseCopy> = {
  'file-between-machines': {
    ...
    guideLabel: 'Without a server, in full',
  },
  'team-drop-box': {
    ...
    guideLabel: 'Setting up a server, in full',
  },
  'ci-backups': {
  ...
  'new-device': {
    ...
    guideLabel: 'Move to a new device, in full',
  },
  'continuous-backup': {
    ...
    guideLabel: 'Continuous backup, in full',
  },
  'encrypted-sync': {
    ...
    guideLabel: 'End-to-end encrypted sync, in full',
  },
  attachments: {
    ...
    guideLabel: 'Attachments beside the data, in full',
  },
}

const copy: Record<Locale, Record<UseCaseSlug, UseCaseCopy>> = { en: useCasesEn, fr: useCasesFr }

/** The seven use cases, in one language. */
export function useCases(locale: Locale): UseCase[] {
  return base.map((entry) => {
    const words = copy[locale][entry.slug]

    return { ...entry, ...words, guide: { label: words.guideLabel, href: entry.guideHref } }
  })
}

export function findUseCase(locale: Locale, slug: string): UseCase | undefined {
  return useCases(locale).find((useCase) => useCase.slug === slug)
}

export const useCaseSlugs: UseCaseSlug[] = base.map((entry) => entry.slug)
